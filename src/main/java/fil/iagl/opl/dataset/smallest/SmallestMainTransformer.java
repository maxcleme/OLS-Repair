...
public class SmallestMainTransformer extends AbstractProcessor<CtClass<?>> implements Transformer {
  ...
  private void modifyExecAndCreate(CtMethod<?> method, CtClass<?> target) {
    ...
    newExecStatements.add(getFactory().Code().createCodeSnippetStatement("int[] input = new int[4]"));
    newExecStatements.add(getFactory().Code().createCodeSnippetStatement("System.out.println(\"Please enter 4 numbers separated by spaces > \")"));
    ...
    newExecStatements.add(getFactory().Code().createCodeSnippetStatement("System.out.println(smallest(input) + \" is the smallest\")"));
  ...
  private void createMethod(List<CtStatement> newMethodStatements, CtClass<?> target) {
    ...
    CtMethod<Integer> smallestMethod = getFactory().Method().create(target, Sets.newHashSet(ModifierKind.PUBLIC), getFactory().Type().INTEGER_PRIMITIVE, "smallest",
